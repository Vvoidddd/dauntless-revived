...
| `Engine.ini` | Memory lines, `r.EyeAdaptationQuality=0`, an optional forced graphics level, the chat redirect | Memory lines and the chat redirect |
...
| Windows server kit, `Install-DauntlessServer.ps1` | Every run of the installer. `Update-DauntlessServer.ps1` does **not** rewrite these files. | Memory lines and the chat redirect only: no `r.EyeAdaptationQuality`, no graphics lines | Overwrites the whole file | Not touched |
...
| `r.EyeAdaptationQuality` | `0` | `0` turns it off | Turns off 1.4.4's automatic exposure, which on current graphics drivers makes the pre-hunt airship cabin nearly black with blown-out windows. Resolution, textures, lighting and hunt atmosphere stay the same. Close every client and start it again for the change to apply. | The three client writers (launcher, friend kit, host script), always. Not the server kit: game servers render nothing. |
...
With level `-1` it ends after `r.EyeAdaptationQuality=0`. The server kit writes only the first four
lines.
