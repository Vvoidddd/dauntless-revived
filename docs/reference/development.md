...
|---|---|---|
| `UndauntedMetagame/` | The backend the game talks to: accounts and keys, characters, inventory, loadouts, progression, parties, friends, matchmaking, the admin API. | TypeScript, Express 5, SQLite through `better-sqlite3` and Drizzle |
...
|---|---|---|
| Metagame | Accounts and usernames, what each kind of key may do, characters and save history, the database, inventory transactions, loadouts and slots, progression (the default, real mode and stub mode, rank math, the upgrade notice), entitlements, cooldowns and bounties, parties, friends, matchmaking, server status | Each file gets its own empty database in `%TEMP%\undaunted-test-*`, a throwaway signing key pair, `AUTH_MODE=APIKEY` and `NODE_ENV=production`. Logs are silent; set `TEST_LOG_LEVEL` (for example `debug`) to see them. |
| Deploy server | Game-server kinds and the list the metagame reads, the checks on a matchmaking request, loopback-only access | `TEST_LOG_LEVEL` works here too. |
...
|---|---|---|---|
| `Test-KitUnit.ps1` | Every kit script parses in PowerShell 5.1 and is ASCII only; invite strings v1 and v2, including everything that must be refused; certificate fingerprints; TLS pinning against a local test server; `Get-ServerStatus.ps1` with and without an account key; account key files; the chunked-upload helper. | `npm ci` in `UndauntedGateway` first: it runs the gateway's certificate tool. | `-WorkDir` (default `%TEMP%\dr-kit-unit`, deleted at the end), `-Port` (default 62450; 62000-62499) |
...
|---|---|---|
| Startup, routing, request log | `server.ts`, `app.ts`, `logger.ts` | `app.ts` mounts every router; unknown routes are logged and answered 404. |
...
| Progression | `controllers/progressionmode.ts` (real or stub, `PROGRESSION_MODE`), `controllers/realprogression.ts` (tracks, objectives, grants, rank confirmations, the admin seed, the upgrade notice), `controllers/progressionrank.ts` (rank math from `vendor/progression_config.json`), `controllers/progressionevents.ts` (audit rows), `controllers/progression.ts` (encountered content and breadcrumbs), `routes/progression.ts`, `middleware/RealProgressionOnly.ts` | `routes/progression.ts` answers both the real and the stub shapes. Tables in migration `0011_real_progression.sql`. |
...
| Subsystem | Code | Notes |
|---|---|---|
| Game servers | `controllers/gameservers.ts` (ports, Ramsgate and the Dojo, starting processes), `controllers/watchdog.ts`, `routes/gameservers.ts` | Loopback callers only. |
