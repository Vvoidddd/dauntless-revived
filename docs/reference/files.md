...
| Path | What it is |
|:-----|:-----------|
| `settings.json` | The joined server (mode, host, port, name, and the Tailscale share link or the certificate fingerprint; the invite code until it has been used), the install folder, the last fully checked game folder, the graphics preset, windowed mode, the language, and per server the username and whether the key backup was offered. Written atomically through `settings.json.tmp`; invalid fields fall back to defaults when it is read. No account key. |
...
| Path | What it is |
|:-----|:-----------|
| `%LOCALAPPDATA%\DauntlessRevived\Game\` | The default game folder: 410 files, about 10.9 GB, plus the two DLLs in `Archon\Binaries\Win64\`. "Change folder…" picks another one (an absolute path of at most 150 characters; a folder that is not empty and has no `Archon` subfolder gets a `DauntlessRevived` subfolder). While downloading, a file is `<name>.part`; a DLL being copied is `<name>.new`. |
