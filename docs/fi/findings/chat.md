...
`CHAT_NICK_CHECK=log` päästää sääntöjä rikkovan nimimerkin sisään ja kirjoittaa varoitusrivin. Se on
paluukytkin siltä varalta, että oikea peliohjelma hylätään testissä, ei tavallinen asetus.
...
- **Ei uudelleenyhdistämisen silmukoita.** Peliohjelma yhdistää uudelleen heti seuraavalla
  ...
  `<close/>`, vastaamaton ping, korvaaminen, sammutus, liian suuri kehys tai jatkuva väärinkäyttö
  päättää yhteyden, ja jokainen palvelimen päättämä yhteys pidättää tilin seuraavaa kirjautumista 60
  sekuntia, jolloin peliohjelma odottaa 15-45 sekuntia (B `0x14093a3ee`).
- **Istunnot.** Enintään kaksi yhteyttä tiliä kohden; kolmas korvaa pisimpään hiljaa olleen. Uusi
  yhteys pingaa vanhemman, ja se päätetään haamuna, jos se ei vastaa 10 sekunnissa.
...
palvelimelta, molemmat pelaajat näkevät käyttäjänimet. WebSocket-testit vievät kaksi ja kolme pelaajaa
liittymisten, viestien, poistumisten, nimimerkkisääntöjen, ryhmä- ja kiltahuoneiden, estojen,
...
| Kuinka usein peliohjelma yrittää hylättyä liittymistä uudelleen | tuntematon | Laske `join refused` -rivit. |
