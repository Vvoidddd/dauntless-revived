...
| POST | `/account/mapping` | valinnainen tunniste | Pelin tilien yhdistämishaku (`QueryAccountMappingsEndpoint`), jota peli käyttää kaverihaun jälkeen ja ennen kuin se näyttää ryhmäkutsun; alkuperäinen projekti vastasi 404. Runko on luettelo tunnisteita (pelkkä taulukko tai `externalIds`, `ids`, `accountIds` tai `externalAuthIds`, sekä valinnainen `type`/`externalAuthType`). Tunnisteen kanssa jokainen tämän palvelimen tilin tunniste yhdistetään itseensä: `{"<id>": [{accountId, displayName, type, externalAuthId, externalAuthIdType, externalDisplayName}]}`; muut jätetään pois. Ilman tunnistetta: `{}`. Lokiin kirjataan vain rungon rakenne. Lisätty 22. syyskuuta 2026, ei vielä vahvistettu pelissä. |
...
| Metodi | Polku | Pääsy | Mitä se tekee |
|:-------|:------|:------|:--------------|
| POST | `/candidate/join` | tunniste | Runko `{gameMode, gameArgs, playerHuntId}`. Syötteen tarkistuksen jälkeen se joko pyytää deploy-palvelimelta palvelimen heti (Ramsgate, Dojo, opetusjakso) tai laittaa pelaajan jonoon metsästystä varten. Metsästysryhmä sulkeutuu 4 pelaajalla tai kun 20 sekuntia kuluu ilman uusia tulijoita. Vastaa `{candidateId, gameMode, huntId, status: "MATCHING"}`. 400 virheelliselle syötteelle, tai kun `MATCHMAKING_MODE` on `DISABLED` tai puuttuu. Ryhmän johtajan liittyminen voi viedä koko ryhmän mukanaan. |
...
vastaanottajaa kohden, ja kutsut vanhenevat 5 minuutissa.
