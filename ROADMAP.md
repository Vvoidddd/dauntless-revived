...
- [ ] **1.15 First friends night** (S) — *Owner decision (2026-09-21): the first friends night runs on a **rented Windows Server with a public IP** (4.10) in **public mode** (1.17), not over Tailscale and not on the owner's PC.*
  ...
  - **What:**
    - Log server memory, CPU and free memory into a CSV every minute. `tools/log-performance.ps1` is a lightweight first sampler; the fuller metrics, daily rotation and capacity report remain in 4.12. The two-player session is untested.
...
- [ ] **3.10 Text chat** (M if the presence/XMPP server exists, L otherwise) — ***Status 2026-09-22: not built.** The first friends test runs without it; use Discord meanwhile. Friends' online status waits for the same service.* *Owner request (2026-09-21): wanted in-game. The 1.4.4 client uses XMPP multi-user chat rooms ("MUC State", "Known Chatrooms", `[OnlineSubsystemMcp.OnlineChatMcp]`). Local/friend scripts and the desktop launcher already point XMPP at the selected private host on port 61099 instead of Epic; there is no listener there yet. Options: a small authenticated service, or an existing XMPP server with a plugin for our login tokens. The authentication handshake and the names of party, Ramsgate and whisper rooms still need capture with two throwaway accounts before implementing or exposing a listener. Planned after parties (1.9); Discord covers voice and chat until then.*
