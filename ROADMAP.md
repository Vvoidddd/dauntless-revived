...
| Guild | No | *Update 2026-09-22: built (3.11): guilds, members and invites are stored in SQLite and survive a restart. Not deployed or tried in game yet.* |
| Party, friends list | No, being built elsewhere | *Update 2026-09-22: built on the server side (1.9). Friendships and blocks are saved in SQLite; parties and invites live in memory, so a metagame restart leaves everyone in a party of one. Tested with simulated players only. The fixes for what blocked them in the two-player test are built, not deployed.* |
