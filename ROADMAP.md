...
- [ ] **3.10 Text chat** (M if the presence/XMPP server exists, L otherwise) — ***Owner request 2026-09-22: show players as online in game** (the Social panel shows everyone, yourself included, as "Offline" without it). Wanted next, once parties and friends work (1.9, 1.11): a small XMPP presence server on the port the launcher already points the game at (61099, through the launcher relay in public mode), then chat.* ***Status 2026-09-22: an off-by-default, loopback-only XMPP prototype passes synthetic two-account tests for JWT login, direct messages and a shared room. It has not been verified with the 1.4.4 game client, does not drive the friends' online indicator, and must not be exposed on the VPS.** Use Discord meanwhile.* *Owner request (2026-09-21): wanted in-game. 1.4.4 typed chat runs on XMPP multi-user chat rooms (the client has "MUC State", "Known Chatrooms", `[OnlineSubsystemMcp.OnlineChatMcp]`). The game's XMPP connection already points at this PC (0.5, `ws://127.0.0.1:61099`). Still unknown: the exact 1.4.4 authentication exchange and room names (party, Ramsgate, whispers). Before enabling a listener for players, capture the client's login, room-join and presence stanzas with two throwaway accounts on a test port (for example 62099), and verify token authentication. In public mode 61099 stays loopback-only behind the gateway; in private mode it opens to Tailscale peers only after authentication is confirmed. (Raised by Vvoidddd, [PR #6](https://github.com/mixutin/dauntless-revived/pull/6).)*
  ...
  - **Local client result (2026-09-22):** one 1.4.4 client sent a Ramsgate message through the isolated loopback prototype; the game displayed the sender UID. Changing self-presence to the username stalled the client's room join, so self-presence stays UID while delivered messages use the authenticated account username. Verify that change in game and exchange messages between two clients before marking chat complete or deploying it broadly.
