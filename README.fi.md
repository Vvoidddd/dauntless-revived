...
| Pelaajaryhmät ja kaverilista | Rakennettu, estyy pelissä (korjaus valmisteilla) | Palvelimen puoli: ryhmäkutsut, hyväksyminen ja hylkääminen, johtajaksi nostaminen, poistaminen ja lähteminen, koko ryhmä samalle metsästyspalvelimelle, yhdessä takaisin Ramsgateen, haku nimellä sekä SQLiteen tallentuva kaverilista ja estolista. Läpäisee integraatiotestit simuloiduilla pelaajilla; ei vielä kokeiltu kahdella oikealla peliohjelmalla (se on seuraava testi). Kutsuminen ei vaadi kaveruutta. Kavereiden näkyminen paikalla vaatii chat-palvelimen, jota ei ole vielä rakennettu Kahden pelaajan testissä 22.9.2026 kaverihaku ja ryhmäkutsut pysähtyivät kutsuun `POST /account/mapping`; korjattu vastaus sekä korjaus metsästykseen, joka odotti samaa pelaajaa kahdesti, ovat valmiina mutta eivät vielä palvelimella. |
