...
| Tekstichat ja paikalla olo | Rakennettu, odottaa testiä pelissä (chat); paikalla olo ei vielä | Ramsgaten, metsästysten, ryhmän ja killan chat sekä kuiskaukset käyttäjänimin, metagamen itsensä sisällä; oletuksena pois päältä, kunnes kaksi pelaajaa on kokeillut sitä (palvelinpaketissa `Set-Chat.ps1 -On`). Ensimmäisen chat-palvelimen kirjoitti ja testasi Vvoidddd; miksi se näytti nimien sijaan `UID-...`, ja korjaus: [Tekstichat](https://mixutin.github.io/dauntless-revived/fi/findings/chat.html). Pelaajien näyttämistä paikalla ei ole vielä rakennettu. Käytä sillä välin Discordia |
